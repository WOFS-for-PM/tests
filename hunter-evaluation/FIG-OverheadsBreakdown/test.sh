...
FILE_SYSTEMS=( "NOVA-NOGC" "NOVA" "NOVA-RELAX" "PMFS" "HUNTER-SYNC" "HUNTER" )
FILE_SIZES=( $((64 * 1024)) )
WORK_LOADS=( "seq" )
...
for file_system in "${FILE_SYSTEMS[@]}"; do
    for work_load in "${WORK_LOADS[@]}"; do
        for fsize in "${FILE_SIZES[@]}"; do
            ...
            # ANCHOR - HUNTER 
            if [[ "${file_system}" == "HUNTER" || "${file_system}" == "HUNTER-SYNC" ]]; then
                bash "$TOOLS_PATH"/setup.sh "${file_system}" "dac" "1"
                ...
                sm_update_time=$(nova_attr_time_stats "valid_summary_header" "$output")
                attrlog_update_time=$(nova_attr_time_stats "attr_log_commit" "$output")
                meta_update_time=$((sm_update_time + attrlog_update_time))
            ...
            # ANCHOR - NOVA 
                ...
                data_write_time=$(nova_attr_time_stats "memcpy_write_nvmm" "$output")
            # ANCHOR - NOVA-NOGC
            elif [[ "${file_system}" == "NOVA-NOGC" ]]; then
                bash "$TOOLS_PATH"/setup.sh "NOVA" "main" "1"
                fsize=$(( 1 * 1024 ))
                if [[ "${work_load}" == "seq" ]]; then
                    BW=$(bash "$TOOLS_PATH"/fio.sh /mnt/pmem0/test 4K "$fsize" 1 | grep WRITE: | awk '{print $2}' | sed 's/bw=//g' | "$TOOLS_PATH"/converter/to_MiB_s)
                elif [[ "${work_load}" == "fsync" ]]; then
