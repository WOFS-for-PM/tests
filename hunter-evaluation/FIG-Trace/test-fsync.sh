...
FILE_SYSTEMS=( "HUNTER-SYNC" "HUNTER-J" "PMFS" "NOVA" "NOVA-RELAX" "SplitFS-FILEBENCH" "EXT4-DAX" "XFS-DAX" )
    ...
    for file_system in "${FILE_SYSTEMS[@]}"; do
        for trace_name in "${TRACES_NAME[@]}"; do
            if [[ "${file_system}" == "SplitFS-FIO" ]]; then
                ...
                # OPS=$(sudo LD_PRELOAD=$BOOST_DIR/libnvp.so "$TOOLS_PATH"/traces/fiu-trace/replay -f "$TRACE_DIR"/"$trace_name"/trace.syscall.tailored -o syscall -m fiu-no-content -d /mnt/pmem0 -s | grep "OPS: " | awk '{print $9}')
                OPS=$(sudo LD_PRELOAD=$BOOST_DIR/libnvp.so python3 "$TOOLS_PATH"/traces/fiu-trace/replay.py "$TRACE_DIR"/"$trace_name"/trace.syscall.tailored /mnt/pmem0 1 | grep "OPS: " | awk '{print $2}')
            else
                ...
                echo "Replaying $trace_name on $file_system"
                # OPS=$(sudo python3 "$TOOLS_PATH"/traces/fiu-trace/replay -f "$TRACE_DIR"/"$trace_name"/trace.syscall.tailored -o syscall -m fiu-no-content -d /mnt/pmem0 -s | grep "OPS: " | awk '{print $9}')
