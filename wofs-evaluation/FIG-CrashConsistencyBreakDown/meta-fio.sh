...
if [ -f "$TABLE_NAME_NOVA" ]; then
    # do nothing
    echo ""
else
    table_create "$TABLE_NAME_NOVA" "workloads D(ns) mle(ns) mt(ns) GC(ns)"
fi

TABLE_NAME_PMFS="$ABS_PATH/performance-comparison-table-PMFS"
if [ -f "$TABLE_NAME_PMFS" ]; then
    # do nothing
    echo ""
else
