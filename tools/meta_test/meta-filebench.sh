...
WORKLOAD=$TOOLS_PATH/fbscripts/fileserver.f

FILE_SYSTEMS=( "NOVA" "PMFS" "HUNTER")

TABLE_NAME="$ABS_PATH/metadata-trace-fileserver-results"
table_create "$TABLE_NAME" "file_system meta_read(bytes) meta_write(bytes) meta_total(bytes) meta_time(ns) meta_times data_write(bytes) data_read(bytes) data_write_time(ns) data_read_time(ns) IOps(ops/s)"
...
for file_system in "${FILE_SYSTEMS[@]}"; do
    if [[ "${file_system}" == "NOVA" ]]; then
         bash "$TOOLS_PATH"/setup.sh "$file_system" "meta-trace" "0"
    ...
    IOps=$(sudo $FILEBENCH -f "$WORKLOAD" | grep "IO Summary" | awk -F'[[:space:]]' '{print $6}')
    ...
    table_add_row "$TABLE_NAME" "$file_system $meta_read_size $meta_write_size $meta_total_size $meta_time \
