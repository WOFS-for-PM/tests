...
# FILE_SYSTEMS=("NOVA" "KILLER" "NOVA-RELAX" "PMFS" "EXT4-DAX" "XFS-DAX")
FILE_SYSTEMS=("KILLER" "PMFS")
...
for job in "${NUM_JOBS[@]}"; do
    ...
    for file_system in "${FILE_SYSTEMS[@]}"; do
        ...
        if [[ "${file_system}" =~ "SplitFS" ]]; then
            ...
            sudo bash "$TOOLS_PATH"/setup.sh "$file_system" "dev" "1"
        else
            sudo bash "$TOOLS_PATH"/setup.sh "$file_system" "main" "1"
        fi
        echo "Running with $job threads"
        load_workload loada_1M $file_system $job

        if [[ "${file_system}" == "KILLER" ]]; then
            cat /proc/fs/HUNTER/pmem0/timing_stats >/home/deadpool/HUNTER-REPO/LOG-KILLER.txt
        else
            umount /mnt/pmem0
            dmesg | tail -n 100 >/home/deadpool/HUNTER-REPO/LOG-PMFS.txt
        fi
