...
dmesg | tail -2 > output 
virt_start=$(cat output | grep "pm_range_detect: get_nvmm_info: dev $disk_name" | awk '{print $9}')
virt_end=$(cat output | grep "pm_range_detect: get_nvmm_info: dev $disk_name" | awk '{print $11}' | sed 's/,//')
